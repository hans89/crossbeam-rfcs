...
original work-stealing deque, we do not assume the reader have read it.
...
proposes a dynamic-sized circular many-stealing deque, which provides the `steal_many()` method that
steals up to half the works in a deque. In addition, for ensuring its correctness, we prove the
...
In this section, we present a pseudocode of deque with the `steal_many()` method, and prove that (1)
it is safe in the C/C++ memory consistency model, and (2) it is functionally correct, i.e. it acts
like a many-stealing deque.
...
A many-stealing deque is owned by the "owner" thread (`Deque<T>`), which can push items to and pop
items from the "bottom" end of the deque. Other threads, which we call "stealers" (`Stealer<T>`),
try to steal items from the "top" end of the deque. Due to CAS failure, a `steal()` or
`steal_many()` invocation may return `Retry`, which basically means the invocation did nothing and
you should retry. Deque's interface is summarized as follows:
...
The pseudocode for `Inner<T>`'s `fn push()`, `fn pop()`, `fn steal()`, and `fn steal_many()` is as
follows:
...
pub fn push(&self, value: T) {
    ...
    'L109: buffer.write(b, value, Relaxed);
    'L110: self.bottom.store(b + 1, Release);

    'M111: if self.max_bottom.get() < b + 1 { self.max_bottom.set(b + 1); }
}
...
pub fn pop(&self) -> Option<T> {
    ...
    'L203: fence(SeqCst);
    'M204: let buffer = self.buffer.load(Relaxed, epoch::unprotected());
    'M205: let mut t = self.top.load(Relaxed);
    'M206: let mut len = b - t;
    ...
    'M211:     while 2 <= len {
    'M212:         match self.top.compare_and_swap_weak(t, t + 2 * buffer.capacity(), Release, Relaxed) {
    'M213:             Err(t_cur) => { t = t_cur; len = b - t; }
    'M214:             Ok(_) => {
    'M215:                 self.bottom.store(b - 1 + 2 * buffer.capacity());
    'M216:                 self.max_bottom.store(b - 1 + 2 * buffer.capacity());
    'M217:                 break 'irregular;
    'M218:             }
    'M219:         }
    'M220:     }

    'M221:     if len == 1 && self.top.compare_and_swap(t, t + 1, Release, Acquire).is_ok() {
    'M222:         self.bottom.store(b, Relaxed);
    'M223:         self.max_bottom.store(b, Relaxed);
    'M224:         return Some(buffer.read(t, Relaxed));
    'M225:     }

    'M226:     self.bottom.store(b, Relaxed);
    'M227:     return None;
    'M228: }

    'M229: if len < buffer.get_capacity() / 4 {
    'M230:     self.resize(buffer.get_capacity() / 2);
    'M231: }

    'M232: Some(buffer.read(b - 1, Relaxed))
}
...
pub fn steal(&self) -> Steal<T> {
    ...
    'M4031: let mut len = b - t;
    'M4032: if len < -1 { len = len + (-len + 1).next_power_of_two(); }
    'L404: if len <= 0 { return Empty; }

    'L405: let buffer = self.buffer.load(Acquire, &guard);
    'L406: let value = buffer.read(t, Relaxed);
...
pub fn steal_many(&self) -> Steal<Vec<T>> {
    'L501: let mut t = self.top.load(Relaxed);

    'L502: let guard = epoch::pin_fence(); // epoch::pin(), but issue fence(SeqCst) even if it is re-entering

    'L503: let b = self.bottom.load(Acquire);

    'M5031: let mut len = b - t;
    'M5032: if len < -1 { len = len + (-len + 1).next_power_of_two(); }
...
## Comparison to prior work on many-stealing deques
...
circular. Furthermore, the workers in these algorithms perform CAS more often than the workers in
...
[deque-pr-many]: https://github.com/jeehoonkang/crossbeam-deque/tree/many-stealing
...
[rfc-deque]: https://github.com/jeehoonkang/crossbeam-rfcs/blob/deque-proof/text/2018-01-07-deque-proof.md
